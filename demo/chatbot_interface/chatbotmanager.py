...
botPath = "/".join(settings.BASE_DIR.split('/')[:-1])
sys.path.append(botPath)

# from faq_api import faq
from qa_api_v2 import FaqSearch
...
class ChatbotManager(AppConfig):
    ...
    def callBot(sentence):
        ...
        if ChatbotManager.bot:
            return ChatbotManager.bot.bot(sentence)
        else:
